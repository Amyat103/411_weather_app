...
def test_create_duplicate_user(session, sample_user):
    ...
    Users.create_user(**sample_user)
    with pytest.raises(
        ValueError, match="User with username 'testuser' already exists"
    ):
        Users.create_user(**sample_user)
...
def test_check_password_correct(session, sample_user):
    ...
    Users.create_user(**sample_user)
    assert (
        Users.check_password(sample_user["username"], sample_user["password"]) is True
...
def test_check_password_incorrect(session, sample_user):
    ...
    Users.create_user(**sample_user)
    assert (
        Users.check_password(sample_user["username"], "wrongpassword") is False
...
def test_update_password(session, sample_user):
    ...
    assert (
        Users.check_password(sample_user["username"], new_password) is True
...
def test_add_favorite_location(session, sample_user):
    """
    Test successfully adding a favorite location for a user.
    """
    # Create a user in the database
    Users.create_user(**sample_user)
...
def test_delete_favorite_locatioN(session, sample_user):
    ...
    Users.remove_favorite_location(sample_user["username"], location)
