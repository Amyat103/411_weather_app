...
def create_app(config_class=ProductionConfig):
    ...
    def delete_user() -> Response:
        ...
        try:
            ...
            if not username:
                return make_response(
                    jsonify({"error": "Invalid input, username is required"}), 400
                )
            ...
            return make_response(
                jsonify({"status": "user deleted", "username": username}), 200
            )
        except Exception as e:
    ...
    def logout():
        ...
        try:
            ...
            return (
                jsonify({"message": f"User {username} logged out successfully."}),
                200,
            )
            ...
            app.logger.error(
                "Error during logout for username %s: %s", username, str(e)
            )
    ...
    def update_password() -> Response:
        ...
        try:
            if not Users.check_password(username, current_password):
                app.logger.warning(
            ...
            app.logger.warning(
                "Password update failed for username %s: %s", username, str(e)
            )
            return jsonify({"error": str(e)}), 400
        except Exception as e:
            app.logger.error(
    ...
    def get_location_by_id(location_id: int) -> Response:
        ...
        try:
            ...
            return make_response(
                jsonify({"status": "success", "location": location}), 200
            )
        except Exception as e:
    ...
    def get_location_by_name(location_name: str) -> Response:
        ...
        try:
            ...
            if not location_name:
                return make_response(
                    jsonify({"error": "Location name is required"}), 400
                )
    ...
    def add_location_to_favorites() -> Response:
        ...
        try:
            ...
            if not location_name:
                return make_response(
                    jsonify({"error": "Invalid input. Name of location is required."}),
                    400,
                )
